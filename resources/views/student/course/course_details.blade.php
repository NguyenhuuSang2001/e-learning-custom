        ...
        <div class="">
            ...
            <div class="row">
                <div class="col-md-12 col-sm-12 col-xs-12">
                    ...
                    <div class="x_panel">
                        <div class="x_title">
                            <h2>{{ $teacherCourse->course->title }} Details</h2>
                            ...
                            <div class="col-md-5 col-sm-5 col-xs-12" style="border:0px solid #e5e5e5;">
                                ...
                                <div class="">
                                    ...
                                    <ul class="list-inline prod_size">
                                        @foreach ($teacherCourseLessons as $lesson)
                                            <li>
                                                <button type="button"
                                                    class="btn btn-default btn-xs">{{ $lesson->title }}</button>
                                            </li>
                                        @endforeach
                                    </ul>
                                </div>
                                ...
                                <div class="">
                                    <div class="product_price">
                                        <h1 class="price">Author : {{ $teacherCourse->teacher->user->name }}</h1>
                                        <input type="hidden" id="teacher_id"
                                            value="{{ $teacherCourse->teacher->user->id }}">
                                ...
                                <div class="col-md-12">
                                    @if ($courseTaken)
                                        @if ($studentCourseTaken->status == \App\Libraries\Enumerations\CourseStudentStatus::$INCOMPLETE)
                                            <a href="{{ route('getCourseLessonsForStudent', ['teacher_course_id' => $teacherCourse->id]) }}"
                                                class="btn btn-default btn-lg">Continue Study</a>
                                            <a href="{{ route('getCourseExamsForStudent', ['teacher_course_id' => $teacherCourse->id]) }}"
                                                class="btn btn-default btn-lg">Exams</a>
                                        @elseif($studentCourseTaken->status == \App\Libraries\Enumerations\CourseStudentStatus::$COMPLETED)
                                            @if ($certificate)
                                                <a target="_blank" class="btn btn-primary btn-flat btn-sm"
                                                    href="{{ asset($certificate->file_path) }}">Download Certificate</a>
                                            @else
                                                Sorry Certificate File Not Found. Please Contact With Course Author.
                                            @endif
                                        @endif
                                    @else
                                        <a href="{{ route('student-course-enroll', ['teacher_course_id' => $teacherCourse->id]) }}"
                                            class="btn btn-default btn-lg">Enroll Now</a>
                                    @endif
                                </div>
